...
def draw_sleep_temp():
    ...
    par1.set_ylabel("Temprature")
    host.set_xlim(time_s[0], time_s[len(time_s)-1])
    par1.set_ylim(0, 30)
    p1, = host.plot(time_s, state, label="sleep")
    p2, = par1.plot(time_t, temp, label="temp")
    host.legend()
...
def draw_sleep_humid():
    ...
    par1.set_ylim(0, 80)
    p1, = host.plot(time_s, state, label="sleep")
    p2, = par1.plot(time_t, humid, label="humid")
    host.legend()
    host.axis["left"].label.set_color(p1.get_color())
    par1.axis["right"].label.set_color(p2.get_color())
    days = mdates.AutoDateLocator()
...
def draw_sleep_heart():
    ...
    host.set_xlabel("Time")
    host.set_ylabel("State")
    par1.set_ylabel("HeartRate")
    host.set_xlim(time_s[0], time_s[len(time_s)-1])
    par1.set_ylim(0, 100)
    p1, = host.plot(time_s, state, label="sleep")
    p2, = par1.plot(time_h, heart, label="heart")
    host.legend()
    host.axis["left"].label.set_color(p1.get_color())
    par1.axis["right"].label.set_color(p2.get_color())
    days = mdates.AutoDateLocator()
...
def draw_heart_temp():
    ...
    plt.subplots_adjust(right=0.75)
    host.set_xlabel("Time")
    host.set_ylabel("HeartRate")
    par1.set_ylabel("Temprature")
    host.set_xlim(time_s[0], time_s[len(time_s)-1])
    host.set_ylim(0, 100)
    par1.set_ylim(0, 35)
    p1, = host.plot(time_h, heart, label="heart")
    p2, = par1.plot(time_t, temp, label="temp")
    host.legend()
    host.axis["left"].label.set_color(p1.get_color())
    par1.axis["right"].label.set_color(p2.get_color())
